...
class HtmlRichTextParser extends StatelessWidget {
  ...
  static get _supportedElements =>
      List()..addAll(_supportedStyleElements)..addAll(_supportedSpecialtyElements)..addAll(_supportedBlockElements);
    ...
    widgetList.forEach((dynamic w) {
      if (w is BlockText) {
        ...
        if ((childTextSpan.text == null || childTextSpan.text.isEmpty) &&
            (childTextSpan.children == null || childTextSpan.children.isEmpty)) return;
      ...
      // handle specialty elements
        ...
        switch (node.localName) {
          ...
          case "table":
            ...
            nextContext.rootWidgetList
                .add(Container(margin: EdgeInsets.symmetric(vertical: 12.0), child: nextContext.parentElement));
            break;
          ...
          case "th":
            ...
            nextContext.childStyle = nextContext.childStyle
                .merge(TextStyle(fontWeight: (node.localName == 'th') ? FontWeight.bold : FontWeight.normal));
          ...
          case "caption":
            ...
            RichText text = RichText(
                textAlign: TextAlign.center, textScaleFactor: 1.2, text: TextSpan(text: '', children: <TextSpan>[]));
            Expanded cell = Expanded(
            ...
            if (showImages) {
              if (node.attributes['src'] != null) {
                final width = imageProperties?.width ??
                    ((node.attributes['width'] != null) ? double.tryParse(node.attributes['width']) : null);
                final height = imageProperties?.height ??
                    ((node.attributes['height'] != null) ? double.tryParse(node.attributes['height']) : null);
                ...
                if (node.attributes['src'].startsWith("data:image") && node.attributes['src'].contains("base64,")) {
                  ...
                  parseContext.rootWidgetList
                      .add(Stack(alignment: Alignment.topCenter, overflow: Overflow.visible, children: [
                    Positioned(
                        left: -10.0,
                        right: -10.0,
                        child: GestureDetector(
            ...
            BlockText blockText = BlockText(
              ...
              child: RichText(
                text: TextSpan(
                  ...
                  children: <TextSpan>[TextSpan(text: '', style: nextContext.childStyle)],
                ),
              ),
            );
            parseContext.rootWidgetList.add(blockText);
